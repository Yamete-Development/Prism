...
Currently, Prism only discovers a broadcast copy is gone when it tries to edit/delete and gets a 404 (code 10008). The dead message cache is populated reactively. This plan makes it proactive, preventing bursts of 404s that can trigger Cloudflare backpressure via `InvalidRequestTracker`.
