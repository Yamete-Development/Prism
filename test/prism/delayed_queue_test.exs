defmodule Prism.DelayedQueueTest do
  ...
  describe "enqueue/2" do
    test "enqueues payload into zset with correct timestamp score", %{redix_conn: redix_conn} do
      ...
      # Check item score
      [item_json, score_str] = Redix.command!(redix_conn, ["ZRANGE", @zset_key, "0", "-1", "WITHSCORES"])
      
      score = String.to_integer(score_str)
      assert score >= now + delay_ms
      assert score <= now + delay_ms + 100 # Add 100ms tolerance for execution time
    ...
    test "publishes a wakeup event if the enqueued item is the earliest" do
      ...
      assert_receive {:redix_pubsub, _, _, :message, %{channel: @pubsub_channel, payload: msg1}}, 1000
      ...
      assert_receive {:redix_pubsub, _, _, :message, %{channel: @pubsub_channel, payload: msg3}}, 1000
