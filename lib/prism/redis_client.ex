defmodule Prism.RedisClient do
  @moduledoc """
  Custom Redis client for OffBroadwayRedisStream that uses standard `command`
  ...
  def create_group(id, config) do
    ...
    case command(pid, cmd) do
      {:ok, _} -> :ok
      {:error, %Redix.Error{message: "BUSYGROUP" <> _}} -> :ok
      result -> result
    end
  end
  ...
  def ack(ids, config) do
    ...
    # Dragonfly doesn't support CLIENT REPLY OFF, so we just run a regular command
    # and ignore the result to simulate noreply safely.
  ...
  def delete_consumers(consumers, config) do
    ...
    case Redix.pipeline(pid, commands) do
      {:ok, _} -> :ok
      {:error, _} = error -> error
    end
  end
